...
Responsibilities:
- Orchestrate Numerology (Pillar 1) + Market Sentiment (Pillar 3) correlation
- Generate Oracle predictions for the next trading day
- Blend crypto/news streams into the alignment score
- Backtest stored recommendations against next-day sentiment development
- Provide trading recommendations (Buy/Hold/Sell/Caution)
"""
...
from correlation_matrix import build_correlation_matrix, find_patterns, get_sentiment_prediction_for_day
from data_bridge import load_merged_data, load_pillar3_sentiment
...
def generate_prediction(target_date: Optional[str] = None) -> Dict[str, Any]:
    """Generate an Oracle prediction for a specific target date."""
    if target_date is None:
        target_date = get_next_trading_day()

    print(f"[ORACLE] Generating prediction for: {target_date}")
    ...
    print(f"[ORACLE] Day Numerology: {day_number} ({'Master' if is_master else 'Standard'})")

    print(f"[ORACLE] Loading {CONFIG['oracle']['correlation_days']} days of historical data...")
    ...
    print("[ORACLE] Building correlation matrix...")
    correlation_matrix = build_correlation_matrix(merged_data_sorted)

    print("[ORACLE] Analyzing patterns...")
...
def calculate_day_alignment_score(
    day_number: int,
    correlation_matrix: Dict[int, Dict[str, Any]],
) -> float:
    """Calculate base alignment score for a specific day number."""
...
def determine_sentiment_trend(recent_data: List[Dict[str, Any]]) -> Dict[str, Any]:
    """Determine sentiment trend from recent merged Oracle records."""
...
def calculate_confidence(alignment_score: float, significance: str) -> float:
    """Calculate confidence based on alignment score and statistical significance."""
...
def generate_trading_recommendation(
    ...
    """Generate trading recommendation based on all available factors."""
...
def build_reasoning(
    day_number: int,
    day_description: str,
    alignment_breakdown: Dict[str, Any],
...
def generate_fallback_prediction(
    ...
    """Generate fallback prediction when insufficient data is available."""
    return {
        "timestamp_utc": datetime.now(timezone.utc).isoformat(),
        "target_date": target_date,
        "oracle_version": CONFIG["oracle"]["version"],
...
def load_backtest_history() -> Dict[str, Any]:
    """Load persistent Oracle backtesting history."""
...
def build_sentiment_lookup(sentiment_history: List[Dict[str, Any]]) -> tuple[Dict[str, Dict[str, Any]], List[str]]:
    """Map sentiment history to Oracle date strings."""
...
def classify_actual_trend(sentiment_delta: float) -> str:
    """Map actual next-day sentiment delta to a trend label."""
...
def normalize_recommendation_direction(recommendation: str) -> str:
    """Normalize Oracle recommendation text into a comparable direction class."""
...
def upsert_backtest_entry(history: List[Dict[str, Any]], prediction: Dict[str, Any]) -> None:
    """Insert or update the latest prediction snapshot by target date."""
    snapshot = {
...
def evaluate_backtest_history(history: List[Dict[str, Any]], sentiment_history: List[Dict[str, Any]]) -> None:
    """Evaluate all pending predictions once actual next-day sentiment is available."""
...
def summarize_backtest_history(history: List[Dict[str, Any]]) -> Dict[str, Any]:
    """Summarize Oracle accuracy from the persistent backtesting history."""
    ...
    if accuracy_pct > 95:
        print("[WARN] High Accuracy detected - potential backfill bias")
...
def update_backtesting(prediction: Dict[str, Any]) -> Dict[str, Any]:
    """Persist prediction history, evaluate ready entries and return summary."""
...
def save_prediction(prediction: Dict[str, Any], output_dir: Optional[str] = None) -> str:
    """Save Oracle prediction to a timestamped JSON artifact."""
    ...
    print(f"[INFO] Prediction saved to: {filepath}")
    return str(filepath)


def test_oracle_prediction():
    """Generate Oracle prediction for the next trading day and print a summary."""
    print("=" * 60)
    print("MBRN ORACLE PROGNOSIS")
    print("=" * 60)

    prediction = generate_prediction()
    print(f"\nTarget Date: {prediction['target_date']}")
    print(f"Day Numerology: {prediction['day_numerology']['day_number']} ({prediction['day_numerology']['description']})")

    if prediction["correlation_summary"].get("status") != "fallback":
        print(f"\nCorrelation Analysis ({CONFIG['oracle']['correlation_days']} days):")
        patterns = prediction["correlation_summary"]
        if patterns.get("strongest_correlation"):
            strongest = patterns["strongest_correlation"]
            print(
                f"- Strongest: Tag {strongest['day_number']} "
                f"(Alignment: {strongest['alignment_score']}, Avg Sentiment: {strongest['avg_sentiment']})"
            )
        if patterns.get("weakest_correlation"):
            weakest = patterns["weakest_correlation"]
            print(
                f"- Weakest: Tag {weakest['day_number']} "
                f"(Alignment: {weakest['alignment_score']}, Avg Sentiment: {weakest['avg_sentiment']})"
            )
        print(f"- Overall Trend: {patterns['overall_trend']}")
        print(f"- Statistical Significance: {patterns['statistical_significance']}")

    print("\nPREDICTION:")
    print(f"- Alignment Score: {prediction['prediction']['alignment_score']}/100")
    print(f"- Confidence: {prediction['prediction']['confidence']:.0%}")
    print(f"- Sentiment Forecast: {prediction['prediction']['sentiment_prediction']}/100")
    print(f"- Trading Recommendation: {prediction['prediction']['trading_recommendation']}")
    print(f"- Oracle Accuracy: {prediction['backtesting']['accuracy_pct']:.2f}%")
    print(f"\nReasoning: {prediction['prediction']['reasoning']}")

    saved_path = save_prediction(prediction)
    print("\n" + "=" * 60)
    print(f"Prediction saved: {saved_path}")
    print("=" * 60)
    return prediction
...
def run() -> Dict[str, Any]:
    """
    Worker entry point for Sentinel integration.

    Generates Oracle prediction and mirrors it to shared/data/ for dashboard consumption.
    """
    print("[ORACLE WORKER] Starting Oracle Prognosis Engine...")
    prediction = generate_prediction()
    saved_path = save_prediction(prediction)
    ...
    print(f"[ORACLE WORKER] Dashboard data mirrored to: {dashboard_path}")
    print(f"[ORACLE WORKER] Artifact saved to: {saved_path}")
    return prediction
