...
One-off history reconstruction for Oracle backtesting and correlation bootstrapping.

Goals:
- Fetch 90 days of daily history for SPY, QQQ and BTC-USD via yfinance
- Reconstruct a pseudo sentiment score (0-100) from daily performance
- Persist records in the same JSON shape as market_sentiment_fetcher
- Trigger oracle_core after backfill so the correlation matrix updates immediately
"""
...
from data_bridge import load_merged_data
...
def pseudo_sentiment_from_performance(weighted_change_percent: float) -> int:
    """
    Convert daily weighted performance into a 0-100 pseudo sentiment.
...
def recommendation_from_score(score: int) -> str:
    """Map pseudo sentiment score into fetcher-style recommendation text."""
    if score >= 70:
        return "buy"
    if score <= 30:
        return "sell"
...
def fetch_history() -> Dict[str, Any]:
    """Download daily OHLCV history for all requested symbols separately."""
    frames: Dict[str, Any] = {}
...
def get_ticker_frame(history: Dict[str, Any], ticker: str) -> Any:
    """Return the already separated symbol frame."""
...
def build_market_entry(frame: Any, current_index: Any, previous_index: Any, ticker: str, fetched_at: datetime) -> Dict[str, Any]:
    """Build one fetcher-compatible market_data entry for a specific day."""
...
def build_enrichment(market_data: List[Dict[str, Any]], fetched_at: datetime) -> Dict[str, Any]:
    """Reconstruct enrichment payload using daily performance heuristics."""
...
def build_record(market_data: List[Dict[str, Any]], fetched_at: datetime) -> Dict[str, Any]:
    """Assemble a market_sentiment_fetcher-compatible record."""
...
def persist_record(record: Dict[str, Any]) -> Path:
    """Persist one historical record to AI/models/data."""
...
def run_backfill() -> List[Path]:
    """Generate historical daily sentiment artifacts."""
...
def trigger_oracle() -> Dict[str, Any]:
    """Trigger oracle_core after backfill so correlation data updates immediately."""
    oracle_dir = PROJECT_ROOT / "scripts" / "oracle"
    if str(oracle_dir) not in sys.path:
        sys.path.append(str(oracle_dir))

    from oracle_core import run
...
def reconstruct_oracle_backtest_history() -> Dict[str, Any]:
    """
    Rebuild Oracle backtesting history from the newly backfilled sentiment artifacts.

    Strategy:
    - Load merged numerology + sentiment history
    - Replay Oracle predictions on rolling historical windows
    - Evaluate each replayed recommendation against the actual next-day sentiment movement
    """
...
def main() -> int:
    print("=" * 70)
    print("MBRN ORACLE BACKFILL - HISTORICAL SENTIMENT RECONSTRUCTION")
    print("=" * 70)

    saved_paths = run_backfill()
    print(f"[OK] Backfill created {len(saved_paths)} historical records")
    if saved_paths:
        print(f"[OK] First artifact: {saved_paths[0]}")
        print(f"[OK] Last artifact:  {saved_paths[-1]}")

    summary = reconstruct_oracle_backtest_history()
    print(
        f"[OK] Oracle replay backtest seeded: {summary['evaluated_predictions']} predictions, "
        f"accuracy {summary['accuracy_pct']:.2f}%"
    )

    prediction = trigger_oracle()
    print(
        "[OK] Oracle refreshed:",
        prediction["target_date"],
        prediction["prediction"]["trading_recommendation"],
        f"Accuracy={prediction['backtesting']['accuracy_pct']:.2f}%",
    )
    return 0
