...
function initForm() {
  ...
  /* Date auto-format: inserts dots automatically (TT.MM.JJJJ) */
  dateInput.addEventListener('input', function(e) {
    ...
    /* Only update if value changed to avoid caret jump on delete */
    if (e.target.value !== formatted) {
      const pos = e.target.selectionStart;
      e.target.value = formatted;
      /* Keep caret at sensible position */
  ...
  form.addEventListener('submit', async e => {
    ...
    /* ── Loading-Feedback ── */
    if (calcBtn) {
      calcBtn.classList.add('btn--calculating', 'btn--loading');
      calcBtn.disabled = true;
      calcBtn.querySelector('.btn-spinner-wrap').textContent = '↻';
    }
    /* Yield to browser so spinner paints before calculations block */
    ...
    /* ── Leerzustand ausblenden ── */
    const emptyEl = document.getElementById('resultsEmpty');
    if (emptyEl) emptyEl.style.display = 'none';

    /* ── Scroll zu Ergebnissen (smooth, nach kurzer Pause) ── */
    setTimeout(() => {
      const anchor = document.getElementById('resultsAnchor') || document.getElementById('lifeHeroDisplay');
      if (anchor) anchor.scrollIntoView({ behavior: 'smooth', block: 'start' });
    }, 350);

    /* ── LIFE HERO DISPLAY befüllen ── */
    (function() {
      ...
      if (heroNum) {
        ...
        /* Reset animation cross-browser: remove class, force reflow, re-add */
        heroNum.classList.remove('num-animate');
        void heroNum.offsetWidth; /* force reflow */
    ...
    /* Hide share/cta until results ready */
    const sbw  = document.getElementById('shareBarWrap');
    const ctaw = document.getElementById('ctaBarWrap');
    if (sbw)  sbw.hidden  = true;
    if (ctaw) ctaw.hidden = true;
    const eg = document.getElementById('extraNumbersGrid');
    if (eg) eg.innerHTML = '';
    ...
    if (calcBtn) {
      calcBtn.classList.remove('btn--calculating', 'btn--loading');
      calcBtn.disabled = false;
      calcBtn.querySelector('.btn-spinner-wrap').textContent = '✦';
    }

    /* ── Teaser-Satz unter Sekundär-Tiles (nicht auf lifeTile, hat eigenen Hero) ── */
    [['expressionTile','expression',expressionVal],
     ['soulTile','soul',soulVal],['personalityTile','personality',personalityVal]]
    ...
    /* ── Pulse-Hint auf klickbaren Tiles (einmalig, nach 1.2s) ── */
    setTimeout(() => {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* Focus name input on load (desktop only — avoids keyboard popup on mobile) */
