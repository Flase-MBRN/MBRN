...
function buildShareText() {
  ...
  let text = '✦ Numerologie-Profil';
  if (name) text += ' von ' + name;
  if (date) {
    try {
      const lifeVal = formatLifePathComponent(date);
      const { base } = parseDisplayValue(lifeVal);
      const meaning  = EXPLANATIONS.life[base] || '';
      text += '\n\nLebenszahl: ' + lifeVal + (meaning ? ' – ' + meaning : '');
    } catch(e) {}
  }
  text += '\n\nBerechne deine Numerologie (36 Zahlen):';
...
function initForm() {
  ...
  resetBtn?.addEventListener('click', () => {
    requestAnimationFrame(() => {
      ...
      /* Remove teasers and tap hints */
      document.querySelectorAll('.result-teaser, .tile-tap-hint').forEach(el => el.remove());
  ...
  form.addEventListener('submit', async e => {
    ...
    /* ── Loading-Feedback ── */
    if (calcBtn) {
      calcBtn.classList.add('btn--calculating');
    ...
    /* Hint sichtbar machen */
    const hintEl = document.getElementById('resultsHint');
    if (hintEl) { hintEl.textContent = '✦ Tippe auf eine Zahl für die vollständige Deutung'; hintEl.style.display = 'block'; }
    /* Hide share/cta until we have results */
    ...
    if (calcBtn) {
      calcBtn.classList.remove('btn--calculating');
      calcBtn.disabled = false;
      calcBtn.querySelector('.btn-spinner-wrap').textContent = '✦';
    }

    /* ── Teaser-Satz unter Kernzahlen ── */
    [['lifeTile','life',lifeVal],['expressionTile','expression',expressionVal],
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  introToggle?.addEventListener('click', () => {
    ...
    if (expanded) {
      introDetails.hidden = true;
    } else {
      introDetails.hidden = false;
    }
  });
