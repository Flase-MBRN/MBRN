...
/**
 * v4.0: targetGridId routes tiles into the correct accordion.
 * Falls back to 'acc-talents-grid' when omitted.
 */
...
/** v4.0: appendGridLabel now targets a specific accordion grid */
...
/** Quantum Score Tile (v4.0 with routing) */
...
function openModal(type, displayValue) {
  ...
  if (type === 'quantum') {
    /* Try to get the full result object from the tile's data attribute */
    const clickedTile = document.querySelector('[data-modal-type="quantum"][data-quantum-result]');
    let result = null;
    if (clickedTile) {
      try { result = JSON.parse(clickedTile.getAttribute('data-quantum-result')); } catch(e) {}
    }
