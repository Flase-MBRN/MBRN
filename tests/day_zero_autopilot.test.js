...
describe('day zero autopilot', () => {
  ...
  test('autopilot runs collector before local llm worker and handles exit codes', () => {
    ...
    expect(source).not.toContain('http://localhost:11434/api/tags');
    expect(source).toContain('-UseBasicParsing');
    expect(source).toContain('ArgumentList @("-u", $scoutScript, "--infinite")');
    expect(source).toContain('$env:PYTHONIOENCODING = "utf-8"');
    expect(source).toContain('$env:PYTHONUTF8 = "1"');
    expect(source).toContain('Horizon Scout bootstrap starting unbuffered UTF-8 process');
    expect(source).toContain('Wait-OllamaReady -TimeoutSeconds 30');
    expect(source).not.toContain('Wait-OllamaReady -TimeoutSeconds 120');
    expect(source).toContain('"--infinite"');
    expect(source).not.toContain('"--loop"');
    expect(source).not.toContain('-RedirectStandardError $scoutLogFile');
    expect(source).toContain('Dry run enabled; Mission Control launch skipped');
    expect(source).toContain('$llmExit -ne 0');
